...
- **Admin ML Training Hub (AdminMLTrainingHub.tsx):** Comprehensive admin dashboard with 5 tabs (Datasets, Jobs, Models, Devices, Consent) for managing ML training datasets, creating/monitoring training jobs, deploying models, extracting device data with consent verification, and viewing consent analytics.
- **Medical NLP Dashboard (MedicalNLPDashboard.tsx):** AI-powered document analysis with medical document upload, text extraction, entity recognition (conditions, medications, procedures, anatomy, lab values, dosages), PHI detection with redaction toggle, and Q&A chat interface for querying medical documents.
- **Enhanced Research Center (ResearchCenter.tsx):** Epidemiology research hub with 4 tabs (Cohort, Studies, AI Analysis, Reports) featuring cohort demographics visualization (gender, condition, age distribution), study management with create/view capabilities, AI-powered report generation, and enrollment trends.
