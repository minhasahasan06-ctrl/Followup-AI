...
- **PainTrack Platform Added (Latest)**: Comprehensive chronic pain tracking system integrated as 6th tab in Daily Follow-up Dashboard. Features module selection (ArthroTrack for arthritis, MuscleTrack for muscle pain, PostOpTrack for post-surgery recovery), joint selection (knee, hip, shoulder, elbow, wrist, ankle), laterality tracking (left/right/bilateral), guided movement instructions, self-reported VAS pain slider (0-10), medication tracking with checkbox and details field, and optional patient notes. Database schema includes paintrack_sessions (dual-camera video URLs, pain VAS, metadata), session_metrics (MediaPipe BlazePose/FaceMesh ML features, anomaly detection, correlation analysis), and clinician_notes tables. Legal constraint: Pain is ALWAYS self-reported - ML metrics are technical features only (joint angles, facial movements, anomaly scores) and NEVER inferred as pain levels. Dual-camera support noted for iPhone 17 Pro.
